#!/usr/bin/env python3
"""
Xior Lutherse Burgwal deep monitor — popup-aware version.

Reads text from the popup (role=dialog, or Xior's modal containers),
not the background page, so a content change INSIDE the popup triggers
an alert.
"""
...
def capture_popup_text(page) -> str:
    """
    Read ONLY the popup/dialog, not the background page.
    Tries multiple strategies and returns whichever finds the most content.
    """
    candidates: list[str] = []

    # Strategy 1: role=dialog
    try:
        locs = page.get_by_role("dialog").all()
        for loc in locs:
            try:
    ...
    # Strategy 2: common modal CSS classes
    css_candidates = [
        '[role="dialog"]',
        '[aria-modal="true"]',
        '.modal:visible',
    ...
    # Strategy 3: element that contains "No rooms available" / "Get notified" / "Select your room"
    try:
        for phrase in [r"no\s+rooms?\s+available", r"get\s+notified", r"select\s+your\s+room"]:
            try:
                loc = page.get_by_text(re.compile(phrase, re.I)).first
                # walk up a few ancestors to capture the whole popup
                for levels in (2, 3, 4, 5):
                    try:
                        ancestor = loc.locator(f"xpath=ancestor::*[{levels}]")
                        t = ancestor.text_content(timeout=1500) or ""
                        if t.strip():
                            candidates.append(t)
                    except Exception:
                        continue
    ...
    # Pick the candidate that is the most distinct vs. body length
    # (i.e., shortest that still contains meaningful content — avoids the full-body fallback)
    candidates = [re.sub(r"\s+", " ", c).strip() for c in candidates if c and c.strip()]
    candidates = [c for c in candidates if 20 < len(c) < 5000]

    if candidates:
        # prefer ones containing a signal phrase
        signal_re = re.compile(
            r"no\s+rooms?\s+available|get\s+notified|select\s+your\s+room|"
            r"start\s+booking|add\s+to\s+cart|select\s+contract",
            re.I,
        )
        signal_hits = [c for c in candidates if signal_re.search(c)]
        pool = signal_hits if signal_hits else candidates
        # shortest meaningful text = most likely the popup, not a wrapper
        best = min(pool, key=len)
        return best

    # Last-resort fallback: body text
    try:
...
def run_flow(page, room: dict) -> dict:
    ...
    captured = capture_popup_text(page)
    log(f"  [popup capture] len={len(captured)} sample={captured[:400]!r}")
